...
I would consider this a useful model with the points in mind that it is a statistically significant model on the whole, and that over half of the variability in either scenario can be accounted for using the model itself. Additionally, two of the three coefficients are either significant or close to it. 
...
Wins appear to be the most impacted by the amount of points scored - I don't know sports very well, but this makes sense to me considering you need more points to win. It seems like wins are the least impacted by yards gained, which implies that successful drives do not necessarily lead to higher occurrences of scoring in football. This is a little more surprising than the last statistic - I think most people would assume yardage and winning have a stronger positive correlation. 
...
This model would be useful for covering a sports team or event because it can account for how many different circumstances are affecting outcomes in real time. For example, if we used this model to cover the Chicago Bears in a season where they have the highest yardage of any team, we could show the lack of correlation between yards gained and overall wins to demonstrate why this factor isn't helpful to the team. Similarly, the model could explain why the Buffalo Bills are doing so well against the Bears in a given game - they have the highest number of points scored in the league (in this scenario). 
