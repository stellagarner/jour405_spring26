...
One thing we'll do a lot in this class is load some CSV data using the `read_csv()` function that the tidyverse provides. Follow along here: https://r4ds.hadley.nz/data-import
and put the code below that loads the students.csv file from the URL provided:
...
R is saying it cannot find the function for reading the CSV data provided. I checked the tidyverse library file that opened for me and it does not seem like the readr package is something provided in it, but I'm not sure if that's where the issue lies. 
